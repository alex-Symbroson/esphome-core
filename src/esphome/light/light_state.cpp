...
LightColorValues LightCall::validate_() {
  ...
  if (this->brightness_.has_value() && !traits.has_brightness()) {
  ...
  if (this->white_.has_value() && !traits.has_rgb_white_value()) {
    ESP_LOGW(TAG, "'%s' - This light does not support setting white value!", name);
    this->white_.reset();
  }

  // Color temperature exists check
  if (this->color_temperature_.has_value() && !traits.has_color_temperature()) {
...
#define VALIDATE_RANGE(name, upper_name) VALIDATE_RANGE_(name, upper_name)
  ...
  bool supports_transition = traits.has_brightness();
...
LightCall &LightCall::set_transition_length_if_supported(uint32_t transition_length) {
  if (this->parent_->get_traits().has_brightness())
...
LightCall &LightCall::set_red_if_supported(float red) {
  if (this->parent_->get_traits().has_rgb())
    this->set_red(red);
  return *this;
}
...
LightCall &LightCall::set_white_if_supported(float white) {
  if (this->parent_->get_traits().has_rgb_white_value())
    this->set_white(white);
  return *this;
}
LightCall &LightCall::set_color_temperature_if_supported(float color_temperature) {
  if (this->parent_->get_traits().has_color_temperature())
